...
import { renderCustomIntegrations } from "./modules/custom-integrations.js";
...
document.addEventListener("DOMContentLoaded", () => {
  // Small delay to ensure DOM is fully parsed and elements are queryable
  setTimeout(() => {
    console.log('[INIT] Starting initialization...');
    cacheRefs();
    console.log('[INIT] Refs cached:', {
      implementation: !!refs.implementation,
      integrations: !!refs.integrations,
      sessionModel: !!refs.sessionModel
    });
    hydrateForm(store);
    console.log('[INIT] Form hydrated');
    bindEvents(store);
    store.subscribe((state) => render(state));
    store.subscribe(() => renderCustomIntegrations(store));
      ...
      if (refs && refs.quotesList) {
        ...
        const loadCallback = async (quoteId) => {
          try {
            ...
            if (result.success && result.data) {
              ...
              toast.success(`Cotización cargada: ${quote.client_name}`);
            }
          } catch (error) {
        ...
        const duplicateCallback = async (quoteId) => {
          try {
            ...
            if (result.success) {
              toast.success(`Cotización duplicada: ${result.data.client_name}`);
